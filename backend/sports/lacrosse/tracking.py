"""Lacrosse tracking evidence (Sprint 12).

Pure, framework-free functions that derive lacrosse-specific supporting evidence
from a detector's ``RawDetections``: which player is carrying the ball, a
per-frame possession timeline, possession changes, the ball's own movement, and a
coarse attacking direction. The genuinely sport-agnostic primitives (player
trajectory grouping and temporally-robust movement labelling) are reused from
``services/extractors/basketball_vision.py`` rather than re-implemented — the only
lacrosse-specific piece is ball-label handling and the lacrosse framing of the
summary.

Consistent with the other sports, tracked detections are SUPPORTING EVIDENCE
...
# Lacrosse footage may label the ball "lacrosse ball"; detectors commonly reuse
# the COCO "sports ball" class for it.
_BALL_LABELS = {"sports ball", "lacrosse ball", "ball"}

# Fraction of the ball's net horizontal drift above which the attack has a clear
...
def summarize_tracked_evidence(detections: RawDetections | None) -> dict | None:
    """Tracking-grounded lacrosse evidence for the adjudicator (additive).
