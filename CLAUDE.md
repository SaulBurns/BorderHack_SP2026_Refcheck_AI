...
- **Health** (`services/health.py`): `liveness()` / `readiness()` pure functions; readiness degrades (`503`) rather than raising when a dependency (provider key, ffmpeg, upload dir) is missing. The `provider_comms` check (Sprint 16D) reports transport health from `provider_health()` — degraded after a real provider call exhausts its retries. **Sprint 17C:** the `provider` check now also verifies the **Gemini `google-genai` SDK** is installed (not just the key) and is **failover-aware** — a degraded primary with a usable `PROVIDER_FALLBACK` stays green (detail names the failover target); readiness also carries a top-level `ai` block (`provider`/`model`/`fallback`/`gemini_sdk_available`) from `services/ai/startup.py:ai_config_summary()`. The `checks` key set is unchanged (stable contract).
...
**Environment configuration:** `AI_PROVIDER` selects the provider (`mock` | `anthropic` | `gemini` | `router`; default `mock`; invalid values raise a clear error). `anthropic` needs `ANTHROPIC_API_KEY` (model override: `AI_MODEL`, default `claude-sonnet-4-5`). `gemini` needs `GEMINI_API_KEY` and the `google-genai` package (model override: `GEMINI_MODEL`, default `gemini-2.5-flash`). Missing keys degrade to the mock fallback with the reason surfaced in diagnostics; an unsupported `AI_PROVIDER` value fails loudly. **Gemini deployment (Sprint 17C):** `google-genai` is now a first-class dependency in `backend/requirements.txt` (still lazily imported), so `AI_PROVIDER=gemini` ships ready-to-run in the Docker image; `render.yaml` carries `GEMINI_API_KEY`/`GEMINI_MODEL`/`PROVIDER_FALLBACK`. At boot, `main.py` runs `services/ai/startup.py:validate_ai_config()` and logs a warning per problem (missing key, missing SDK, unsupported provider/fallback) plus an `ai_config_summary()` info line — non-fatal, but the misconfig is visible before the first request. `GeminiProvider.sdk_available()` is the pure SDK probe (delegates to `startup.gemini_sdk_available()` → `importlib.util.find_spec`), used by validation, readiness, and diagnostics (never imports the SDK). **Graceful failover (Sprint 17C):** `PROVIDER_FALLBACK` names a secondary provider; if the primary raises a `ProviderError` (permanent, or transient retries exhausted) inside `_send_messages`, the same turn is retried once on the fallback (via the shared `_call_provider` helper) before the pipeline degrades to the mock — e.g. a Gemini primary fails over to Anthropic. Unset = unchanged behavior. Model resolution per provider lives in `config.resolved_model_for_provider(name)`. **Provider router (Sprint 17A):** `AI_PROVIDER=router` selects `RouterProvider` (`services/ai/providers/router_provider.py`), which picks a **different provider per task** — perception (vision) vs adjudication (reasoning) — via `ROUTER_PERCEPTION_PROVIDER` / `ROUTER_ADJUDICATION_PROVIDER` (each falling back to `ROUTER_DEFAULT_PROVIDER`, default `anthropic`, so out of the box it behaves like anthropic). Routing happens at the `_send_messages` seam through the `AIProvider.route(task)` method (default returns `self`; only the router overrides it): `_send_validated` derives the task from the response model (`PerceptionResponse`→perception, `AdjudicatorResponse`→adjudication) and the delegate that actually runs is what's wrapped in `call_with_retry` — so reliability diagnostics report the real provider, never "router". No other provider and **no sport plugin** ever learns routing happened (enforced by `tests/test_provider_router.py`). **Per-agent models (Sprint 17B):** the router now also pins a **model per stage** — `PERCEPTION_PROVIDER` + `PERCEPTION_MODEL` and `ADJUDICATOR_PROVIDER` + `ADJUDICATOR_MODEL` (e.g. perception → Gemini Flash, adjudication → Claude Sonnet). The concise stage-provider names take precedence over the legacy `ROUTER_*` provider vars (still honored for backward compatibility); a model left unset lets the resolved provider pick its own (`AI_MODEL`/`GEMINI_MODEL` env or built-in default), so an empty router config is byte-for-byte the Sprint 17A behavior. Resolution lives in config (`router_provider_for(task)` / `router_model_for(task)`); `route(task)` binds the model by wrapping the delegate in a transparent `_ModelBoundProvider` (only when a model is set) that injects `model=` into `send_messages` and reports the pinned model via `model_name()` while still exposing the delegate's real `provider_name()`/`is_mock`. The `model` override is a provider-neutral optional param on `send_messages` (like `temperature`): Anthropic/Gemini honor it over their env/default, the mock ignores it. `describe_routing()` returns `{task: {provider, model}}`. Enforced by `tests/test_per_agent_models.py` (mixed-provider + mixed-model pipeline) and the sport-abstraction guard in `tests/test_provider_router.py`. **Structured outputs (Sprint 16B):** when the pipeline validates a reply it passes a `response_schema` to `send_messages`. **Gemini** always honors it — `response_mime_type=application/json` + `response_schema` (native JSON mode + schema). **Anthropic** native `output_config.format` is **opt-in** via `ANTHROPIC_STRUCTURED_OUTPUT=1` (default OFF, because the default model `claude-sonnet-4-5` does not guarantee support — off = "robust JSON mode": JSON-only prompt + Pydantic validation + retry); enable it only on a supporting model (Opus 4.8 / Sonnet 5 / Haiku 4.5). `STRUCTURED_OUTPUT_RETRIES` (default 1) tunes retries. **Transport reliability (Sprint 16D):** every provider call goes through `services/ai/reliability.py:call_with_retry` (wrapped in `_send_messages`), which retries **only transient** failures (429/5xx/529, timeout, dropped connection) with exponential backoff + jitter, and never retries permanent failures (400 bad request, 401 auth, 403/404) or validation failures (those are a separate retry in `_send_validated`). Providers raise classified `TransientProviderError`/`PermanentProviderError` (`services/ai/errors.py`); on exhaustion the error message names provider, model, attempt count, and latency (→ `fallback_reason`), and a process-local `provider_health()` record feeds the `provider_comms` readiness check. Tunable via `PROVIDER_MAX_RETRIES` (default 2), `PROVIDER_BACKOFF_BASE_SECONDS` (0.5), `PROVIDER_BACKOFF_MAX_SECONDS` (8.0), `PROVIDER_TIMEOUT_SECONDS` (90). **Opt-in provider optimizations (Sprint 14, default OFF):** `ANTHROPIC_PROMPT_CACHE=1` enables Anthropic prompt caching of the system prompt; `GEMINI_JSON_MODE=1` requests JSON-typed Gemini output (superseded by the schema path when validating). All are pure request-builder changes (`build_payload` / `generation_kwargs`) that leave default behavior identical.
...
**YOLO version + config (Sprint 18A):** the YOLO detector runs the latest stable Ultralytics flagship — **YOLO26** (`yolo26n.pt`, up from YOLOv8-nano) — pinned via `ultralytics>=8.4.104,<8.5` (optional dep; imported lazily). The `yolov8` registry key/name is kept as a **stable role identifier** (no breaking change to `DETECTOR=yolov8`, `DETECTOR=hybrid`, tests, or diagnostics); the concrete weights that ran are reported on `RawDetections.model`. Everything is env-configurable via `config.resolved_yolo_*`: `YOLO_MODEL` (default `yolo26n.pt` — e.g. `yolo11n.pt`/`yolov8n.pt`), `YOLO_CONFIDENCE` (0.25), `YOLO_TRACKER` (`bytetrack.yaml`), `YOLO_TRACKING` (on). `DETECTOR_VERSION` bumped `0.2.0`→`0.3.0`. `services/detectors/benchmark.py` provides the before/after metrics — IoU detection precision/recall/F1 (`detection_metrics`), tracking quality (`tracking_metrics`: ID switches, track purity, matched ratio), and latency; `scripts/yolo_benchmark.py` runs them (`--live --before yolov8n.pt --after yolo26n.pt` for a real head-to-head; offline self-check by default). Committed artifact: `evaluation/reports/yolo_benchmark.{md,json}`.
